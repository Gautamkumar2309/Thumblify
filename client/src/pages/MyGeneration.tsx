...
const MyGeneration = () => { 
    ...
    <>
        ...
        <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
            {/* HEADER */}
            <div>
                <h1 className="text-2xl font text-zinc-200">My Generations</h1>
                <p className="text-sm text-zinc-400 mt-1">View and manage all your AI-generated thumbnails</p>
            </div>
            {/*LOADING*/}
            {loading && (
                <div className="grid grid-cols-1 sm:grid-co1s-2 lg:grid-cols-3 gap-6">
            ...
            {!loading && thumbnails.length > 0 && (
                <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
                    {thumbnails.map((thumb: IThumbnail)=>{
                        ...
                        return(
                            <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-x1 break-insode-avoid">
                                {/* Image */}
                                ...
                                <div onClick={(e)=>e.stopPropagation()}className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                                    ...
                                    className="size-6 bg-black/50 p-1 rouded hover:bg-pink-600 transition-all"/>
                                    ...
                                    <ArrowRightIcon className="size-6 bg-black/50 p-1 rouded hover:bg-pink-600 transition-all"/>
                                    </Link>
                                </div>
                            </div>
                        )
